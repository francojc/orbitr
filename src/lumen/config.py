...
Config file location: ~/.config/lumen/config.toml (XDG Base Directory spec).
...
CONFIG_DIR: Path = (
    Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "lumen"
)
CACHE_DIR: Path = (
    Path(os.environ.get("XDG_CACHE_HOME", Path.home() / ".cache")) / "lumen"
)
