"""orbitr search — keyword and field-filtered search across sources."""
...
from orbitr.config import VALID_FORMATS, VALID_SOURCES, Config
...
from orbitr.core.ranking import rank
from orbitr.display import effective_format, render
from orbitr.exceptions import LumenError, NoResultsError, SourceError, UsageError
...
def search(
    ...
    Examples:
      ...
      orbitr search "transformer attention"

      orbitr search "BERT" --sources semantic_scholar --limit 20

      orbitr search "neural machine translation" --author "Vaswani" --from 2017

      orbitr search "title:contrastive learning" --sort citations --format json
    """
        ...
        if isinstance(result, SourceError):
            ...
            errors.append(
                SourceError(
                    ...
                    suggestion="Run `orbitr doctor` to check source connectivity.",
                )
            )
